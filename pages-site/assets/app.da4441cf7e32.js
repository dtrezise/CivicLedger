...
class DataLoadError extends Error {
  constructor(path, status) {
    super(`Unable to load ${path}${status ? ` (${status})` : ""}`);
    this.name = "DataLoadError";
    this.code = status ? `HTTP_${status}` : "NETWORK_ERROR";
    this.resource = path;
...
async function fetchJson(pathOrRecord) {
  ...
  try {
    ...
    throw new DataLoadError(path, null);
  }
  if (!response.ok) throw new DataLoadError(path, response.status);
  try {
    return await response.json();
  } catch (_error) {
    const error = new DataLoadError(path, response.status);
...
function freshnessFor(value) {
  const generated = Date.parse(`${value || ""}T00:00:00Z`);
  if (!Number.isFinite(generated)) {
    return { ageDays: null, className: "unknown", label: "Freshness unknown", detail: "No valid dataset date is available." };
  }
  const ageDays = Math.max(0, Math.floor((Date.now() - generated) / 86400000));
  if (ageDays <= 3) {
    return {
      ageDays,
      className: "current",
      label: "Dataset current",
      detail: ageDays === 0 ? `Generated ${formatDate(value)}.` : `Generated ${formatDate(value)}, ${ageDays} day${ageDays === 1 ? "" : "s"} ago.`,
    };
  }
  if (ageDays <= 7) {
    return {
      ageDays,
      className: "aging",
      label: "Dataset refresh due",
      detail: `Generated ${formatDate(value)}, ${ageDays} days ago. Scheduled sources may be awaiting refresh.`,
    };
  }
  return {
    ageDays,
    className: "stale",
    label: "Stale dataset warning",
    detail: `Generated ${formatDate(value)}, ${ageDays} days ago. Verify source freshness before relying on current-office comparisons.`,
  };
}

function renderFreshnessStatus() {
  const freshness = freshnessFor(state.overview?.generated_at);
  const target = $("datasetFreshness");
  target.hidden = false;
  target.className = `freshness-status ${freshness.className}`;
  target.dataset.ageDays = freshness.ageDays == null ? "unknown" : String(freshness.ageDays);
  target.innerHTML = `<strong>${escapeHtml(freshness.label)}</strong><span>${escapeHtml(freshness.detail)}</span>`;
  $("headerStatus").classList.toggle("warning", ["aging", "stale", "unknown"].includes(freshness.className));
  return freshness;
}
...
function renderLoadFailure(error) {
  ...
  const target = $("loadFailure");
  target.hidden = false;
  target.dataset.errorCode = code;
  $("loadFailureCode").textContent = code;
  $("loadFailureResource").textContent = resource;
  $("loadFailureMessage").textContent =
    code === "INVALID_JSON"
      ? "A required data file arrived in an unreadable format. The release has been stopped at its evidence boundary."
      : "The explorer shell is available, but one or more required records did not arrive. Retry or use the maintained fallback.";
}
...
$("retryDataButton").addEventListener("click", () => window.location.reload());
loadData();
