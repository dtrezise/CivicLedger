...
def bucket_floor(value: date, bucket: str) -> date:
    if bucket == "month":
...
async def get_timeline(
    ...
    if not all_trades:
        return TimelineResponse(bucket=bucket, start=start, end=end, buckets=[], gaps=[])
    ...
    # Build buckets
    buckets = []
    current = bucket_floor(range_start, bucket)
    ...
    return TimelineResponse(bucket=bucket, start=range_start, end=range_end, buckets=buckets, gaps=gaps)
