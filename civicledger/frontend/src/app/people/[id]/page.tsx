...
type ScorecardResponse = {
  grade?: string;
  score?: number;
  reporting?: string; // "Yes/Limited/No"
  median_lag_days?: number;
  disclosure_type?: string; // "transactions"/"holdings"
  notes?: string;
};
...
export default function ProfilePage({
  ...
  useEffect(() => {
    ...
    async function loadSecondary() {
      ...
      // events list — backend currently only has GET /events
      // Try the most likely params first; if your backend uses different query keys,
      // we can adjust after you paste one /events response.
      try {
        const eUrl =
          `${API_BASE}/events?scope=person&person_id=${encodeURIComponent(id)}` +
    ...
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        ...
        <div className="w-full max-w-[180px] rounded-lg border bg-white p-4 text-center shadow-sm">
          ...
          <div className="mt-1 text-xs text-gray-500">Disclosure Score</div>
        </div>
      </div>
      ...
      <div className="mt-6 rounded-lg border bg-white p-4">
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-4">
          <div>
            ...
            <div className="text-sm">{scorecard?.reporting ?? "—"}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500">Median Lag</div>
            <div className="text-sm">{scorecard?.median_lag_days != null ? `${scorecard.median_lag_days} days` : "—"}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="text-sm">{scorecard?.notes ?? "—"}</div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 text-sm">
        ...
        <div className="ml-auto flex items-center gap-3">
          <Link
            href={`/people/${person.id}/trades`}
            className="rounded-md border px-3 py-1.5 text-sm hover:bg-gray-50"
          >
            View Trades
          </Link>
          <Link
            href={`/sharecards/new?person_id=${person.id}`}
