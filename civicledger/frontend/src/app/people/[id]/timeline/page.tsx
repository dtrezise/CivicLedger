...
export default function TimelinePage({
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.getPerson(id),
      api.getTimeline(id, { bucket }),
      api.getMarketSeries({ symbols: "SPY,DIA" }),
      api.getEvents({ scope: "person", person_id: id }),
    ])
      .then(([p, t, m, e]) => {
        setPerson(p);
        setTimeline(t);
  ...
  if (!person) return <p className="text-red-500">Not found.</p>;
  ...
  const chartData = (timeline?.buckets || []).map((b) => {
    ...
    for (const series of marketData) {
      const pt = series.points.find((p) => p.date >= b.start);
              ...
              {showEvents &&
                events.map((evt) => (
                  <ReferenceLine
                    key={evt.event_id}
                    yAxisId="trades"
                    x={bucket === "month" ? evt.date.slice(0, 7) : evt.date}
                    stroke="#94a3b8"
                    strokeDasharray="3 3"
                  />
                ))}
